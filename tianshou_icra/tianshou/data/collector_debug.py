...
class Collector_debug(object):
    ...
    def __init__(self,
        ...
        self._rew_metric = reward_metric or Collector_debug._default_rew_metric
        self.reset()
    ...
    def reset(self) -> None:
        ...
        self.data = Batch(state={}, obs={}, act={}, rew={}, done={}, info={},
                          obs_next={}, policy={}, log_prob={})#, alpha={})#, int_rew={})
        self.reset_env()
        self.reset_buffer()
    ...
    def collect(self,
        ...
        while True:
            if cur_step >= 100000 and cur_episode.sum() == 0:  # 每一个episode都在收集,但我觉得这个有bug,毕竟就算sum不为0也可能出现这个情况?
                warnings.warn(
            ...
            if random:
                ...
                try:
                    with torch.no_grad():
                        result = self.policy(self.data, last_state, deterministic=(self._mode == 'test'))
            ...
            if hasattr(result, 'log_prob'):
                self.data.log_prob = to_numpy(result.log_prob)
            if hasattr(result, 'alpha'):
                self.data.alpha = to_numpy(result.alpha)
            if hasattr(result, 'int_rew'):
                self.data.int_rew = to_numpy(result.int_rew)
