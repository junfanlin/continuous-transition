...
class SACMUTRIRBPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            diff_blend = 1 - (pred_blend.clamp(0, 1) - targ_blend).abs()

            if self.norm_diff:
                diff_blend = diff_blend / diff_blend.sum() * diff_blend.shape[0]
        ...
        critic1_loss = (F.mse_loss(current_q1, target_q, reduction='none') * diff_blend).mean()
        # critic 2
        current_q2 = self.critic2(batch.obs, batch_act)
        critic2_loss = (F.mse_loss(current_q2, target_q, reduction='none') * diff_blend).mean()
        ...
        actor_loss = ((self._alpha * obs_result.log_prob - torch.min(  # if current entropy is small, alpha is large, then log_prob is encourge to larger
            current_q1a, current_q2a)) * diff_blend).mean()
        ...
        if self._automatic_alpha_tuning:
            ...
            alpha_loss = -((self._log_alpha * log_prob) * diff_blend).mean()  # if current entropy is small, alpha will be large
