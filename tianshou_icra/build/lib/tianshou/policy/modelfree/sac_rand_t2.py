...
class SACT2Policy(DDPGPolicy):
    ...
    def forward(self, batch, state=None, input='obs', **kwargs):
        ...
        if kwargs.get('deterministic', False):
            # w = torch.zeros_like(self.w.w)
            w = kwargs.get('w', self.w.w)
            logits, h = self.actor(obs, w, state=state, info=batch.info)
    ...
    def learn(self, batch, **kwargs):
        ...
        for _ in range(getattr(self, 'repeat', 1)):
            w = self.w.sample((batch.obs.shape[0], ))
            # w[-1] = w[-1] * 0 # placehold for exploitation
            ...
            with torch.no_grad():
                ...
                target_q = torch.min(
                    self.critic1_old(batch.obs_next, w, a_),
                    self.critic2_old(batch.obs_next, w, a_),
            ...
            # critic 1
            current_q1 = self.critic1(batch.obs, w, batch_act)
            critic1_loss = F.mse_loss(current_q1, target_q)
            # critic 2
            current_q2 = self.critic2(batch.obs, w, batch_act)
            ...
            a = obs_result.act
            current_q1a = self.critic1(batch.obs, w, a)
            current_q2a = self.critic2(batch.obs, w, a)
        ...
        # w = self.w.sample((batch.obs.shape[0],)).clamp(0, 1)
        # w = w * 0 # placehold for exploitation
        # w = torch.distributions.Normal(torch.zeros(w.shape[-1]), torch.ones(w.shape[-1]) * 0.1).sample((batch.obs.shape[0],)).clamp(0, 1)
        # rew_intrisic = (rew_intrisic_feature * w).mean(-1, keepdim=True).detach()
        #
        # with torch.no_grad():
        #     obs_next_result = self(batch, w=w, input='obs_next')
        ...
        kl_div = 0
        if (self.strict_buf and len(self.buf_actor) == self.buf_len) or (not self.strict_buf and len(self.buf_actor) >= 1):
            # is still not for sure whether to use w_target of zero
            logits_old, _ = self.actor_old(batch.obs, self.w.w.repeat(batch.obs.shape[0], 1))
            logits_ori, _ = self.actor(batch.obs, self.w_target.w.repeat(batch.obs.shape[0], 1).detach())
        ...
        if self.buf_cnt % self.buf_interval == 0:
            self.buf_actor.append(copy.deepcopy(self.actor.state_dict()))
            self.actor_old.load_state_dict(self.buf_actor[0])
