...
class SACSRNDPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        log_alpha = self._log_alpha(rnd_next)
        alpha = log_alpha.exp()
    ...
    def update_rand(self, data, batch_size, times=5):
        ...
        for _ in range(times):
            ...
            for batch in data.split(batch_size):
                ...
                pred_target = self.rand_target(obs_next_normed)[0].detach()
                pred_source = self.rand_source(obs_next_normed)[0]
                rew_intrisic = F.mse_loss(pred_source, pred_target, reduction='none')
                ...
                if self.inverse_rew_intrisic:
                    ...
                    rew_intrisic_mixed = (F.mse_loss(pred_source, pred_target, reduction='none').sum(-1, keepdim=True) - 30).pow(2).mean()

                self.rand_optim.zero_grad()
                if self.inverse_rew_intrisic:
                    (rew_intrisic.sum(-1).mean() + 0.1 * rew_intrisic_mixed).backward()
                else:
                    rew_intrisic.sum(-1).mean().backward()
                self.rand_optim.step()

                rns_list.append(rew_intrisic.sum(-1).mean().item())
    ...
    def update_rms(self, data, batch_size, update=False):
        ...
        return batch_int.sum(-1), batch_alpha