...
class SACRTPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.class_num = kwargs.get('class_num', 1)

        self.mapper = np.random.rand(np.prod(self.actor.state_shape), self.class_num)
    ...
    def learn(self, batch, **kwargs):
        ...
        mapres_obs = torch.FloatTensor(np.dot(batch.obs, self.mapper)).softmax(-1)
        mapres_obs_next = torch.FloatTensor(np.dot(batch.obs_next, self.mapper)).softmax(-1)
        ...
        with torch.no_grad():
            ...
            target_q = torch.min(
                (self.critic1_old(batch.obs_next, a_) * mapres_obs_next).sum(-1, keepdim=True),
                (self.critic2_old(batch.obs_next, a_) * mapres_obs_next).sum(-1, keepdim=True),
        ...
        # critic 1
        current_q1_old = self.critic1(batch.obs, batch_act)
        current_q1 = (current_q1_old * mapres_obs).sum(-1, keepdim=True)
        critic1_loss = F.mse_loss(current_q1, target_q)
        grad_c1 = torch.autograd.grad(critic1_loss, fast_weight_c1, create_graph=True, retain_graph=True)
        fast_weight_c1_update = self.critic1_optim.step(fast_weight_c1, grad_c1, nostate=True)

        current_q1_new = self.critic1(batch.obs, batch_act, vars=fast_weight_c1_update)
        critic1_retain_loss = (((current_q1_new - current_q1_old.detach()).square() * (1 - mapres_obs)).sum(-1) / self.class_num).mean()
        grad_c1 = torch.autograd.grad(critic1_loss + critic1_retain_loss, fast_weight_c1, create_graph=True, retain_graph=True)

        fast_weight_c1 = self.critic1_optim.step(fast_weight_c1, grad_c1)

        # critic 2
        current_q2_old = self.critic2(batch.obs, batch_act)
        current_q2 = (current_q2_old * mapres_obs).sum(-1, keepdim=True)
        critic2_loss = F.mse_loss(current_q2, target_q)
        grad_c2 = torch.autograd.grad(critic2_loss, fast_weight_c2, create_graph=True, retain_graph=True)
        fast_weight_c2_update = self.critic2_optim.step(fast_weight_c2, grad_c2, nostate=True)

        current_q2_new = self.critic2(batch.obs, batch_act, vars=fast_weight_c2_update)
        critic2_retain_loss = (((current_q2_new - current_q2_old.detach()).square() * (1 - mapres_obs)).sum(-1) / (
            self.class_num)).mean()
        grad_c2 = torch.autograd.grad(critic2_loss + critic2_retain_loss, fast_weight_c2, create_graph=True,
                                      retain_graph=True)
        fast_weight_c2 = self.critic2_optim.step(fast_weight_c2, grad_c2)
        ...
        a = obs_result.act
        current_q1a = (self.critic1(batch.obs, a) * mapres_obs).sum(-1, keepdim=True)
        current_q2a = (self.critic2(batch.obs, a) * mapres_obs).sum(-1, keepdim=True)
        ...
        result = {
            ...
            'loss/critic1_retain': critic1_retain_loss.item(),
            'loss/critic2_ratain': critic2_retain_loss.item(),
            'alpha': alpha,
