...
class SACMUTRIRBIBFPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.disc, self.disc_optim = kwargs.get('discriminator', None)
    ...
    def learn(self, batch, **kwargs):
        ...
        if self.beta is not None:
            beta = self.beta(np.concatenate([batch.obs, batch.act, batch.act_next], -1))
        else:
            beta = torch.ones(batch.obs.shape[0], 1)
        ...
        pred_blend = (trans_mix - trans_pred).mean(-1, keepdim=True) / (trans_pred_next - trans_pred).mean(-1, keepdim=True)
        pred_blend = pred_blend.clamp(0, 1)

        pred_blend_numpy = pred_blend.squeeze().data.numpy()

        batch.obs_next = batch.obs_next + pred_blend_numpy[:, None] * (batch.obs_next_next - batch.obs_next)
        batch.rew = batch.rew + pred_blend_numpy * (batch.rew_next - batch.rew)
        batch.done = batch.done + pred_blend_numpy * (batch.done_next * 1. - batch.done)

        diff_blend = (pred_blend - targ_blend).abs()

        if self.beta is not None:
            beta_loss = (beta.log() * (diff_blend - self.tor_diff)).mean()

            self.beta_optim.zero_grad()
            beta_loss.backward()
            self.beta_optim.step()

        ratio = torch.ones_like(diff_blend)#1 - diff_blend.detach()
