...
class SACUNIFYPolicy(DDPGPolicy):
    ...
    def forward(self, batch, state=None, input='obs', actor='actor', **kwargs):
        obs = getattr(batch, input)
        logits, h = getattr(self, actor)(obs, state=state, info=batch.info)
        ...
        act_optimal = (torch.tanh(mu) * self._action_scale).clamp(self._range[0], self._range[1])
        ...
        return Batch(logits=logits, act=act, state=h, dist=dist, log_prob=log_prob, act_optimal=act_optimal)

    def learn(self, batch, **kwargs):
        with torch.no_grad():
            obs_next_result = self(batch, input='obs_next', actor='actor_old')
            a_ = obs_next_result.act
            a_opt_ = obs_next_result.act_optimal
            dev = a_.device
            batch_act = torch.tensor(batch.act, dtype=torch.float, device=dev)
            target_q = torch.min(
                self.critic1_old(batch.obs_next, a_, a_opt_),
                self.critic2_old(batch.obs_next, a_, a_opt_),
        ...
        act = obs_result.act
        a_opt = obs_result.act_optimal

        actor_loss = (self._alpha * obs_result.log_prob).mean()
        # critic 1
        current_q1 = self.critic1(batch.obs, batch_act, act) + actor_loss
        critic1_loss = F.mse_loss(current_q1, target_q)
        # critic 2
        current_q2 = self.critic2(batch.obs, batch_act, act) + actor_loss
