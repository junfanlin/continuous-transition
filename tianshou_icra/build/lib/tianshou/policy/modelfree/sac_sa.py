...
class SACSAPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        batch_val = batch[256:]
        batch = batch[:256]
        if kwargs.get('current_step', 1) == 1:
            self.flip = 0


        # PRETEST
        with torch.no_grad():
            obs_next_result_tmp = self(batch_val, input='obs_next', deterministic=True)
            done_val = torch.FloatTensor(batch_val.done[:, None])
            rew_val = torch.FloatTensor(batch_val.rew[:, None])


            current_q1 = self.critic1(batch_val.obs, batch_val.act)
            current_q2 = self.critic2(batch_val.obs, batch_val.act)
            current_qb = torch.min(current_q1, current_q2)

            current_next_q1 = self.critic1(batch_val.obs_next, obs_next_result_tmp.act)
            current_next_q2 = self.critic2(batch_val.obs_next, obs_next_result_tmp.act)
        ...
        critic1_loss = ((current_q1 - target_q).square()).mean()
        # critic 2
        current_q2 = self.critic2(batch.obs, batch_act, vars=fast_weight_c2)
        critic2_loss = ((current_q2 - target_q).square()).mean()

        grad_c1 = torch.autograd.grad(critic1_loss, fast_weight_c1, create_graph=True, retain_graph=True)
        fast_weight_c1_update = self.critic1_optim.step(fast_weight_c1, grad_c1)

        grad_c2 = torch.autograd.grad(critic2_loss, fast_weight_c2, create_graph=True, retain_graph=True)
        fast_weight_c2_update = self.critic2_optim.step(fast_weight_c2, grad_c2)

        # AFTERTEST
        with torch.no_grad():
            current_q1 = self.critic1(batch_val.obs, batch_val.act, vars=fast_weight_c1_update)
            current_q2 = self.critic2(batch_val.obs, batch_val.act, vars=fast_weight_c2_update)
            current_qb = torch.min(current_q1, current_q2)

            current_next_q1 = self.critic1(batch_val.obs_next, obs_next_result_tmp.act, vars=fast_weight_c1_update)
            current_next_q2 = self.critic2(batch_val.obs_next, obs_next_result_tmp.act, vars=fast_weight_c2_update)
            current_next_q = torch.min(current_next_q1, current_next_q2)

            pred_rew = current_qb - (1. - done_val) * self._gamma * current_next_q
            after_rew_loss = F.mse_loss(pred_rew, rew_val)

        deltaE = (after_rew_loss - pre_rew_loss).item()
        T = 1000 - kwargs.get('current_step', 1) + 1
        K = self.K # smaller K, easier to reject
        prob = np.exp(-1/(K * T)) # prob to accept the transition

        # this way might have affect from Adam optimizer, and should we update or just skip it?
        skip = (deltaE > 0) and (np.random.rand() > prob)
        if skip: # it is worse
            # fast_weight_c1_update = self.critic1_optim.step(fast_weight_c1, [-g for g in grad_c1])
            # fast_weight_c2_update = self.critic2_optim.step(fast_weight_c2, [-g for g in grad_c2])

            self.flip += 1

        else:
            for o, n in zip(self.critic1.parameters(), fast_weight_c1_update):
                o.data.copy_(n.data)

            for o, n in zip(self.critic2.parameters(), fast_weight_c2_update):
                o.data.copy_(n.data)
        ...
        result = {
            ...
            'loss/pre_rew': pre_rew_loss.item(),
            'loss/aft_rew': after_rew_loss.item(),
            'SA/flip': self.flip,
            'SA/current_step': kwargs.get('current_step', 1),
            'SA/deltaE': deltaE,
            'SA/prob': prob
        }
        if self._automatic_alpha_tuning:
