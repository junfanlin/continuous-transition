...
class SACMUTRITAPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.get_beta = kwargs.get('get_beta', None)
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            batch_beta = torch.tensor(batch.beta, dtype=torch.float, device=dev)
            target_q = torch.min(
                self.critic1_old(batch.obs_next, self.get_beta(a_)),
                self.critic2_old(batch.obs_next, self.get_beta(a_)),
        ...
        # critic 1
        current_q1 = self.critic1(batch.obs, self.get_beta(batch_act, batch_act_next, batch_beta))
        critic1_loss = F.mse_loss(current_q1, target_q)
        # critic 2
        current_q2 = self.critic2(batch.obs, self.get_beta(batch_act, batch_act_next, batch_beta))
        ...
        a = obs_result.act
        current_q1a = self.critic1(batch.obs, self.get_beta(a))
        current_q2a = self.critic2(batch.obs, self.get_beta(a))
