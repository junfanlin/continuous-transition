...
class SACFETMPPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        if kwargs.get('current_step', 0) > 990:
            ( (kwargs.get('current_step', 0) % 2 * 2 - 1) * critic1_loss).backward()
        else:
            critic1_loss.backward()
        ...
        if kwargs.get('current_step', 0) > 990:
            ( (kwargs.get('current_step', 0) % 2 * 2 - 1) * critic2_loss).backward()
        else:
            critic2_loss.backward()
        self.critic2_optim.step()

        obs_result = self(batch, input='obs_bk') # obs_bk
        a = obs_result.act
        ...
        with torch.no_grad():
            batch = batch_val
            obs_result_tmp = self(batch, input='obs_next_ori_bk', deterministic=True)
            a = obs_result_tmp.act
            done_ori = torch.FloatTensor(batch.done_ori)[:, None]
            rew_ori = torch.FloatTensor(batch.rew_ori)[:, None]
            current_q = torch.min(self.critic1(batch.obs_ori_bk, batch.act_ori), self.critic2(batch.obs_ori_bk, batch.act_ori))
            current_targ_q = torch.min(self.critic1(batch.obs_next_ori_bk, a), self.critic2(batch.obs_next_ori_bk, a))
            current_rew = current_q - (1 - done_ori) * current_targ_q
            pred_loss = F.mse_loss(current_rew, rew_ori)

            obs_result_tmp = self(batch, input='obs_next_next_ori_bk', deterministic=True)
            a = obs_result_tmp.act
            done_ori = torch.FloatTensor(batch.done_next_ori)[:, None]
            rew_ori = torch.FloatTensor(batch.rew_next_ori)[:, None]
            current_q = torch.min(self.critic1(batch.obs_next_ori_bk, batch.act_next_ori),
                                  self.critic2(batch.obs_next_ori_bk, batch.act_next_ori))
            current_targ_q = torch.min(self.critic1(batch.obs_next_next_ori_bk, a), self.critic2(batch.obs_next_next_ori_bk, a))
            current_rew = current_q - (1 - done_ori) * current_targ_q
            pred_next_loss = F.mse_loss(current_rew, rew_ori)

            sum_loss = pred_loss + pred_next_loss
