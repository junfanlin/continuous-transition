...
class SACSRNDDirectTPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.rnd_param = kwargs.get('rnd_param', 1)
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            rnd_next = F.mse_loss(pred_source, pred_target, reduction='none').sum(-1, keepdim=True)
            ...
            rnd_next = self.rnd_process(rnd_next, self.reward_rms.std)
            ...
            rew = torch.tensor(batch.rew,
                               dtype=torch.float, device=dev)[:, None] + self.rnd_param * rnd_next
        ...
        if self._automatic_alpha_tuning:
            log_prob = (
                        obs_result.log_prob + self._target_entropy).detach()  # target_entropy - expected_entropy = target_entropy - (-plogp) = target_entropy + plogp
        ...
        for _ in range(times):
            ...
            for batch in data.split(batch_size):
                ...
                if self.inverse_rew_intrisic:
                    ...
                    np.random.shuffle(shuffle_indexes)
                    mix_factor = np.random.rand(obs_next_normed.shape[0], 1)
                    obs_next_normed_mixed = obs_next_normed_mixed[shuffle_indexes] * mix_factor + (
                            1 - mix_factor) * obs_next_normed_mixed
                ...
                if self.inverse_rew_intrisic:
                    ...
                    rew_intrisic_mixed = F.relu(self.rnd_m - F.mse_loss(pred_source, pred_target, reduction='none').sum(-1, keepdim=True)).pow(2).mean()
    ...
    def update_rms(self, data, batch_size, update=False):
        ...
        with torch.no_grad():
            ...
            for batch in data.split(batch_size, shuffle=False):
                ...
                batch_alpha.append(self._alpha.view(1, 1).repeat(rnd_next.shape[0], 1))
