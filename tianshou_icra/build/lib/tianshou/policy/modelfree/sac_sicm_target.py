...
class SACSICMTPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.rndalpha = kwargs.get('rndalpha', False)
        self.rnd_process = kwargs.get('rnd_process', (lambda x : x))
        self.rnd_m = kwargs.get('rnd_m', 20)
        self.rnd_clip = kwargs.get('rnd_clip', 100)
        self.inv_weight = kwargs.get('inv_weight', 0.1)

        self.has_rand = 'rand' in kwargs
        if self.has_rand:
            self.rand_source, self.rand_optim, self.reward_rms, self.obs_rms = kwargs['rand']
            #= RunningMeanStd(shape=self.actor.state_shape) = RunningMeanStd()
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            if self.obs_rms is not None:
                if self.class_input:
                    ...
                    obs_normed = (batch.obs - self.obs_rms.mean) / np.sqrt(self.obs_rms.var + 1e-5)
                    obs_normed = (obs_normed - obs_normed.mean(-1, keepdims=True)) / \
                                      (obs_normed.std(-1, keepdims=True) + self.__eps)
            else:
                obs_next_normed = batch.obs_next
                obs_normed = batch.obs
            ...
            pred_source = self.rand_source(obs_normed, batch.act)[0]
            rnd_next = F.mse_loss(pred_source, obs_next_normed, reduction='none').sum(-1, keepdim=True).clamp(max=self.rnd_clip)
        ...
        if self.rndalpha:
            log_alpha = self._log_alpha(rnd_next.detach())
        else:
            log_alpha = self._log_alpha(obs_next_normed)
        alpha = log_alpha.exp()
        ...
        with torch.no_grad():
            ...
            ) - alpha * obs_next_result.log_prob
        ...
        actor_loss = (alpha.detach() * obs_result.log_prob - torch.min(  # if current entropy is small, alpha is large, then log_prob is encourge to larger
        ...
        log_prob = (obs_result.log_prob + self._target_entropy + rnd_next).detach()  # target_entropy - expected_entropy = target_entropy - (-plogp) = target_entropy + plogp
        alpha_loss = -(log_alpha * log_prob).mean()  # if current entropy is small, alpha will be large
        self._alpha_optim.zero_grad()
        alpha_loss.backward()
        self._alpha_optim.step()
        # self._log_alpha.w.data.clamp_(min=0)
        ...
        result = {
            ...
            'rnd/rnd_mean': rnd_next.mean().item(),
            'rnd/rnd_std': rnd_next.std().item(),
            'rnd/rnd_max': rnd_next.max().item(),
            'rnd/rnd_min': rnd_next.min().item()
        }
        return result
    ...
    def update_rand(self, data, batch_size, times=5):
        ...
        for _ in range(times):
            ...
            for batch in data.split(batch_size):
                ...
                obs_normed = torch.FloatTensor(obs_normed, device=self.actor.device)
                pred_source = self.rand_source(obs_normed, batch.act)[0]
                rew_intrisic = F.mse_loss(pred_source, obs_next_normed, reduction='none').sum(-1, keepdim=True)
    ...
    def update_reward_rms(self, data, batch_size):
        with torch.no_grad():
            ...
            for batch in data.split(batch_size, shuffle=False):
                ...
                obs_normed = torch.FloatTensor(obs_normed, device=self.actor.device)
                pred_source = self.rand_source(obs_normed, batch.act)[0]
                rnd_next = F.mse_loss(pred_source, obs_next_normed, reduction='none').sum(-1, keepdim=True)
    ...
    def update_rms(self, data, batch_size, update=False):
        ...
        with torch.no_grad():
            ...
            for batch in data.split(batch_size, shuffle=False):
                ...
                if self.rndalpha:
                    alpha = self._log_alpha(rnd_next).exp()
                else:
                    alpha = self._log_alpha(obs_next_normed).exp()
