...
class SACICMPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.has_icm = 'icm' in kwargs
        if self.has_icm:
            self.embnet, self.embnet_optim, self.dynamics, self.dynamics_optim = kwargs['icm']
    ...
    def update_rand(self, data, batch_size, times=1):
        for _ in range(times):
            for batch in data.split(batch_size):
                batch_act = torch.FloatTensor(batch.act, device=self.actor.device)
                ...
                pred_emb_next = self.dynamics(self.embnet(batch.obs), batch.act)
                int_rew = (pred_emb_next - self.embnet(batch.obs_next).detach()).pow(2).sum(-1, keepdim=True)

                self.dynamics_optim.zero_grad()
                int_rew.mean().backward()
    ...
    def update_rms(self, data, batch_size):
        with torch.no_grad():
            ...
            for batch in data.split(batch_size):
                pred_emb_next = self.dynamics(self.embnet(batch.obs), batch.act)
                int_rew = (pred_emb_next - self.embnet(batch.obs_next).detach()).pow(2).sum(-1, keepdim=True)
                batch_int.append(int_rew.data.numpy())
        return np.concatenate(batch_int, 0)