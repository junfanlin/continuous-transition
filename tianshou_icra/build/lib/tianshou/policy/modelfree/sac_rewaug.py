...
class SACRAPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            context = ((np.sign(batch.rew) * np.sqrt(np.abs(batch.rew)))[:, None]).astype(np.float32)
            context_t = torch.tensor(context, dtype=torch.float, device=dev)

            target_q = torch.min(
                self.critic1_old(batch.obs_next + context, a_ + context_t),
                self.critic2_old(batch.obs_next + context, a_ + context_t),
        ...
        # critic 1
        current_q1 = self.critic1(batch.obs + context, batch_act + context_t)
        critic1_loss = F.mse_loss(current_q1, target_q)
        # critic 2
        current_q2 = self.critic2(batch.obs + context, batch_act + context_t)
        ...
        a = obs_result.act
        current_q1a = self.critic1(batch.obs + context, a + context_t)
        current_q2a = self.critic2(batch.obs + context, a + context_t)
