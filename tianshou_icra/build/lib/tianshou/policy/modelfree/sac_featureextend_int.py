...
class SACFEINTPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        targ_blend = torch.zeros(1)#torch.FloatTensor(batch.blend)
        if self.disc is not None:
        ...
        with torch.no_grad():
            ...
            a_ = torch.cat([obs_next_result.act, torch.zeros(batch.obs.shape[0], 1)], -1)
        ...
        obs_result = self(batch, input='obs')
        a = torch.cat([obs_result.act, torch.zeros(batch.obs.shape[0], 1)], -1)
        # a = obs_result.act
