...
class SACMUTRI2Policy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            batch_blend = torch.tensor(batch.blend, dtype=torch.float, device=dev)
            target_q = torch.min(
        ...
        # critic 1
        current_q1 = self.critic1(batch.obs, batch_act) * (1 - batch_blend) + self.critic1(batch.obs, batch_act_next) * batch_blend
        critic1_loss = F.mse_loss(current_q1, target_q)
        # critic 2
        current_q2 = self.critic2(batch.obs, batch_act) * (1 - batch_blend) + self.critic2(batch.obs, batch_act_next) * batch_blend
