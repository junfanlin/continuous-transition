...
class SACSTPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        if self._automatic_alpha_tuning:
            self._log_alpha = alpha[0]
            self._alpha_optim = alpha[1]
        else:
            self._alpha = alpha
    ...
    def learn(self, batch, **kwargs):
        # Critic Loss
        if self._automatic_alpha_tuning:
            self._alpha = self._log_alpha._log_alpha.exp()
    ...
    def get_optimizers(self):
        return [self.critic1_optim, self.critic2_optim, self.actor_optim, self._alpha_optim]
    ...
    def gather_evaluation(self, data, batch_size):
        ...
        mu_list = torch.cat(mu_list, 0)
        sigma_list = torch.cat(sigma_list, 0)

        return mu_list, sigma_list

    def sample_w(self):
        return self._log_alpha.sample_w()

    def set_w(self, w):
        self._log_alpha.set_w(w)

    def update_w(self, p1, p2, p_target, batch_size):
        w1, p1 = p1
        w2, p2 = p2
        ...
        with torch.no_grad():
            for start_idx in range(0, _len, batch_size):
                end_idx = np.minimum((start_idx + 1) * batch_size, _len)

                dist_targ = torch.distributions.Normal(m[start_idx: end_idx], s[start_idx: end_idx])
                dist = torch.distributions.Normal(m1[start_idx: end_idx], s1[start_idx: end_idx])
                kl_div = torch.distributions.kl.kl_divergence(dist, dist_targ).mean()
                loss1.append(kl_div.mean())
                dist = torch.distributions.Normal(m2[start_idx: end_idx], s2[start_idx: end_idx])
                kl_div = torch.distributions.kl.kl_divergence(dist, dist_targ).mean()
                loss2.append(kl_div.mean())


            loss1 = torch.stack(loss1).mean()
            loss2 = torch.stack(loss2).mean()
            grad = (loss1 - loss2) / (w1 - w2)

        self._alpha_optim.zero_grad()
        for p in self._log_alpha.parameters():
            p.grad.data.copy_(grad.data * self.grad_scale)
        self._alpha_optim.step()

        return (loss1 + loss2).item() / 2, (loss1 - loss2).item(), grad.mean().item()
