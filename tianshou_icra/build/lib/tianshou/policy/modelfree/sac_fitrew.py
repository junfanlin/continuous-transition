...
class SACFRPolicy(DDPGPolicy):
    ...
    def forward(self, batch, state=None, input='obs', **kwargs):
        ...
        logits, h, a_weight = self.actor(obs, state=state, info=batch.info)
        ...
        return Batch(logits=logits, act=act, state=h, dist=dist, log_prob=log_prob, weight=a_weight)
    ...
    def learn(self, batch, **kwargs):
        ...
        current_q1a_next = self.critic1(batch.obs_next, a_next)
        current_q2a_next = self.critic2(batch.obs_next, a_next)

        sim = (-((batch_act - a).square() * obs_result.weight).sum(-1).sqrt()).exp()

        pred_r = torch.min(current_q1a, current_q2a) * sim -\
                 (1. - done) * self._gamma * torch.min(current_q1a_next, current_q2a_next)
        r_loss = F.mse_loss(pred_r, rew)

        actor_loss = (self._alpha * obs_result.log_prob).mean() + r_loss
