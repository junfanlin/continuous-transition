...
class SACRWPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.reweight_reward = kwargs.get('reweight_reward', False)
        self.dist, self.dist_optim = kwargs.get('dist', (None, None))
    ...
    def learn(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            # if self.reweight_reward:
            #     rew_ = rew * self.dist.scale  # should we enlarge this?
            # else:
            #     rew_ = rew
        ...
        #
        #     opt_rand = (current_qa > current_qb).float() # those qualified
        #
        ...
        #     current_next_q = torch.min(current_next_q1, current_next_q2) - self._alpha * obs_result_tmp.log_prob
        #
        #     pred_rew = current_qb - (1. - done) * self._gamma * current_next_q
        #
        # # pred_rew = pred_rew / self.dist.scale # scale should be positive
        # if opt_rand.sum().item() > 0:
        #     if self.balance:
        #         rew_loss = (F.mse_loss(pred_rew * self.dist.scale, rew,
        #                                reduction='none') * opt_rand).sum() / opt_rand.sum() #+ (
        #                                #self.dist.scale - 1).square().mean()
        #     # else:
        #     #     # Notice here we use rew_ instead of rew
        #     #     rew_loss = (F.mse_loss(rew_ * self.dist.scale, pred_rew,
        #     #                            reduction='none') * opt_rand).sum() / opt_rand.sum() #+ (
        #     #                            #self.dist.scale - 1).square().mean()
        # else:
        #     rew_loss = (self.dist.scale - 1).square().mean()
