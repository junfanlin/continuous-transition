...
class SACUPPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.up_step = kwargs.get('up_step', 0)
        self.flip_critic = kwargs.get('flip_critic', False)
        self.flip_actor = kwargs.get('flip_actor', False)
    ...
    def learn(self, batch, **kwargs):
        ...
        if (kwargs.get('current_step', 1) > (1000 - self.up_step)) and self.flip_critic:
        ...
        if (kwargs.get('current_step', 1) > (1000 - self.up_step)) and self.flip_actor:
            (self._alpha * obs_result.log_prob + torch.min(
                # if current entropy is small, alpha is large, then log_prob is encourge to larger
                current_q1a, current_q2a)).mean().backward()
        else:
            actor_loss.backward()
