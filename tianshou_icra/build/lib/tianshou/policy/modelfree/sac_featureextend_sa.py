...
class SACFESAPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        ################################
        with torch.no_grad():
            obs_result_tmp = self(batch, input='obs_next_pt')
            a = obs_result_tmp.act
            done_ori = torch.FloatTensor(batch.done_pt)[:, None]
            rew_ori = torch.FloatTensor(batch.rew_pt)[:, None]
            current_q = torch.min(
                self.critic1(batch.obs_pt, batch.act_pt), self.critic2(batch.obs_pt, batch.act_pt)
            )
            current_targ_q = torch.min(
                self.critic1_old(batch.obs_next_pt, a), self.critic2_old(batch.obs_next_pt, a)
            ) - self._alpha * obs_result_tmp.log_prob
            current_rew = current_q - (1 - done_ori) * current_targ_q
            pred_loss = F.mse_loss(current_rew, rew_ori)
            # weight = np.exp( -(1000 - kwargs['current_step']) / 10 )
            # baseline = weight * pred_loss
        ...
        noflip = (self.pre_val_loss is None) \
                 or (self.pre_val_loss > pred_loss) \
                 or (kwargs.get('current_step', 1) < 980) \
                 or (np.random.rand() > 0.5)
        self.pre_val_loss = pred_loss

        self.critic1_optim.zero_grad()
        # ((critic1_loss - baseline).abs() + baseline).backward()
        if noflip:
        ...
        # ((critic2_loss - baseline).abs() + baseline).backward()
        if noflip:
        ...
        obs_result = self(batch, input='obs') # obs_bk
        a = obs_result.act
        ...
        result = {
            ...
            'loss/pred_loss': pred_loss.item(),
            # 'loss/baseline': baseline.item(),
            # 'loss/weight': weight,
            'current_step': kwargs['current_step'],
            'flip': self.flip,
            # 'flip/critic2': self.flip2,
        }
        if self.disc is not None:
