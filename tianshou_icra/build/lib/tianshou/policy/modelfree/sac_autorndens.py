...
class SACAUTORNDENSPolicy(DDPGPolicy):
    ...
    def forward(self, batch, state=None, input='obs', **kwargs):
        ...
        #     alpha = self.log_alpha(rnd).exp()

        act = act.clamp(self._range[0], self._range[1])

        return Batch(logits=logits, act=act, state=h, dist=dist, log_prob=log_prob)#, alpha=alpha)
        ...
        with torch.no_grad():
            ...
            if self.class_input:
                ...
                obs_next_normed = (obs_next_normed - obs_next_normed.mean(-1, keepdims=True)) / obs_next_normed.std(-1,
                                                                                                                    keepdims=True)
            obs_next_normed = torch.FloatTensor(obs_next_normed, device=self.actor.device)
            pred_target = self.rand_target(obs_next_normed)[0].detach() # b, emb
            pred_source = self.rand_source(obs_next_normed)[0]  # b, emb, ens
            # rnd_next = F.mse_loss(pred_source, pred_target, reduction='none')
            rnd_next = (pred_source - pred_target.unsqueeze(-1)).pow(2)
            rnd_next = rnd_next.mean(-1) + rnd_next.std(-1)
            ...
            pred_source = self.rand_source(obs_normed)[0]
            # rnd = F.mse_loss(pred_source, pred_target, reduction='none')
            rnd = (pred_source - pred_target.unsqueeze(-1)).pow(2)
            rnd = rnd.mean(-1) + rnd.std(-1)
            ...
            ) - self.log_alpha(rnd_next).exp().clamp(0, 1) * obs_next_result.log_prob
    ...
    def update_rand(self, data, batch_size, times=1):
        ...
        for _ in range(times):
            ...
            for batch in data.split(batch_size):
                ...
                mask = torch.FloatTensor(batch.mask, device=self.actor.device)
                obs_next_normed = torch.FloatTensor(obs_next_normed, device=self.actor.device)
                pred_target = self.rand_target(obs_next_normed)[0].detach()
                pred_source = self.rand_source(obs_next_normed)[0]
                # rew_intrisic = (pred_source - pred_target).pow(2).sum(-1, keepdim=True)
                rnd_next = (pred_source - pred_target.unsqueeze(-1)).pow(2)
                rnd_next_masked = rnd_next * mask.unsqueeze(-2)
                rnd_next = rnd_next.mean(-1) + rnd_next.std(-1)
                rew_intrisic = rnd_next.sum(-1, keepdim=True)

                rew_intrisic_np = rew_intrisic.data.numpy()
                self.reward_rms.update(rew_intrisic_np)

                self.rand_optim.zero_grad()
                rnd_next_masked.mean().backward()
                self.rand_optim.step()
    ...
    def update_rms(self, data, batch_size, update=True):
        ...
        # if self.class_input and self.obs_rms.max is None:
        #     self.obs_rms.update(data.obs_next)
        ...
        with torch.no_grad():
            ...
            for batch in data.split(batch_size, shuffle=False):
                ...
                rnd_next = (pred_source - pred_target.unsqueeze(-1)).pow(2)
                rnd_next = rnd_next.mean(-1) + rnd_next.std(-1)
                log_alpha = self.log_alpha(rnd_next)
                alpha = log_alpha.exp()
                rew_intrisic = rnd_next.sum(-1, keepdim=True) / 2
