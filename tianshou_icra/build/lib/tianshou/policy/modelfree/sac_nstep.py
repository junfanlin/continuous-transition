...
class SACNSPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        with torch.no_grad():
            extendable = (1 - batch.done_bk[:, None]) * 0
            second = (extendable * np.random.randint(0, 2, extendable.shape)).astype(np.float32)

            obs_next = batch.obs_next * (1 - second) + second * batch.obs_next_next
            gamma = self._gamma * (1 - second) + second * self._gamma ** 2
            rew = batch.rew[:, None] * (1 - second) +\
                  second * (batch.rew[:, None] + self._gamma * batch.rew_next[:, None])
            done = batch.done[:, None] * (1 - second) + second * batch.done_next[:, None]
            batch.obs_next_fake = obs_next

            obs_next_result = self(batch, input='obs_next_fake')
            a_ = obs_next_result.act
            dev = a_.device
            target_q = torch.min(
                self.critic1_old(batch.obs_next_fake, [a_]),
                self.critic2_old(batch.obs_next_fake, [a_]),
            ...
            batch_act_next = torch.tensor(batch.act_next, dtype=torch.float, device=dev)
            second = torch.tensor(second, dtype=torch.float, device=dev)

        # critic 1
        current_q1 = self.critic1(batch.obs, [batch_act, batch_act_next], second)
        critic1_loss = F.mse_loss(current_q1, target_q)
        # critic 2
        current_q2 = self.critic2(batch.obs, [batch_act, batch_act_next], second)
