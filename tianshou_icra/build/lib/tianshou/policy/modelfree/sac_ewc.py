...
class SACEWCPolicy(DDPGPolicy):
    ...
    def __init__(self,
        ...
        self.w1 = kwargs.get('w1', 0.5) # yue da yue guan zhu xin de canshu
        self.w2 = kwargs.get('w2', 1)

        self.w_c1 = [torch.zeros_like(param) for param in self.critic1.parameters()]
        self.w_c2 = [torch.zeros_like(param) for param in self.critic2.parameters()]
    ...
    def learn(self, batch, **kwargs):
        ...
        grad_loss_c1 = torch.zeros(1)
        for grad, weight in zip(grad_c1, self.w_c1):
            grad_loss_c1 = (grad ** 2 * weight).sum() + grad_loss_c1
        self.w_c1 = [(weight * (1 - self.w1) + grad ** 2 * self.w1).detach() for grad, weight in zip(grad_c1, self.w_c1)]
        ...
        grad_loss_c2 = torch.zeros(1)
        for grad, weight in zip(grad_c2, self.w_c2):
            grad_loss_c2 = (grad ** 2 * weight).sum() + grad_loss_c2
        self.w_c2 = [(weight * (1 - self.w1) + grad ** 2 * self.w1).detach() for grad, weight in zip(grad_c2, self.w_c2)]

        self.critic1_optim.zero_grad()
        (critic1_loss + grad_loss_c1 * self.w2).backward()
        self.critic1_optim.step()

        self.critic2_optim.zero_grad()
        (critic2_loss + grad_loss_c2 * self.w2).backward()
        ...
        result = {
            ...
            'loss/grad1': grad_loss_c1.item(),
            'loss/grad2': grad_loss_c2.item(),
            'alpha': alpha,
