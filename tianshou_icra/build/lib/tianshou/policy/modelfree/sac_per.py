...
class SACPerPolicy(DDPGPolicy):
    ...
    def learn(self, batch, **kwargs):
        ...
        critic1_loss = batch.critic1_loss
        critic2_loss = batch.critic2_loss
    ...
    def process_fn(self, batch: Batch, buffer: ReplayBuffer,
        ...
        weight = torch.tensor(batch['impt_weight'], dtype=torch.float, device=dev)[:, None]
        # critic 1
        current_q1 = self.critic1(batch.obs, batch_act)
        td_q1 = current_q1 - target_q
        critic1_loss = (F.mse_loss(current_q1, target_q, reduction='none') * weight).mean()
        # critic 2
        current_q2 = self.critic2(batch.obs, batch_act)
        td_q2 = current_q2 - target_q
        critic2_loss = (F.mse_loss(current_q2, target_q, reduction='none') * weight).mean()

        alpha = (td_q1.abs() > td_q2.abs()).float()
        td_max = (alpha * td_q1.abs() + (1 - alpha) * td_q2.abs()).squeeze().data.numpy()

        buffer.update_weight(indice, td_max)

        batch.critic1_loss = critic1_loss
        batch.critic2_loss = critic2_loss
        return batch