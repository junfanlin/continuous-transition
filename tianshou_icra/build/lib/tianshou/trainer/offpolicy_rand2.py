    ...
    for epoch in range(1, 1 + max_epoch):
        ...
        with tqdm.tqdm(total=step_per_epoch, desc=f'Epoch #{epoch}',
            ...
            while t.n < t.total:  # this is shown in the progress bar, represents the update times
                ...
                indexes = []
                cur_ind = train_collector.buffer._index - 1
                for i in range(result['n/st']):
                    indexes = [cur_ind] + indexes
                    cur_ind -= 1
                    if cur_ind < 0:
                        cur_ind = train_collector.buffer._size - 1

                policy.update_rms(train_collector.buffer[np.array(indexes)], batch_size)
                ...
                for _ in range(4):
                    policy.update_rand(train_collector.buffer[np.array(indexes)], result['n/st'])
