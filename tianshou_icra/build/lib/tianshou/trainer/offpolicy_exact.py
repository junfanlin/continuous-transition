...
def offpolicy_exact_trainer(
    ...
    for epoch in range(1, 1 + max_epoch):
        ...
        with tqdm.tqdm(total=step_per_epoch, desc=f'Epoch #{epoch}',
            ...
            while t.n < t.total:  # this is shown in the progress bar, represents the update times
                ...
                for i in range(update_per_step * min(
                    ...
                    update_times += 1
                    current_step += 1
                    losses = policy.learn(train_collector.sample(batch_size), current_step=current_step)
                    for k in losses.keys():
                        if stat.get(k) is None:
                            stat[k] = MovAvg(size=move_avg_size)
                        stat[k].add(losses[k])
        ...
        if epoch in epochs_to_save:
            if save_fn:
                save_fn(policy, name='policy'+str(epoch)+'.pth')
