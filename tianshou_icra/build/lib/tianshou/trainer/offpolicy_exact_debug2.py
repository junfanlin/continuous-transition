...
def offpolicy_exact_debug2_trainer(
        policy: BasePolicy,
        ...
        with tqdm.tqdm(total=step_per_epoch, desc=f'Epoch #{epoch}',
            ...
            while t.n < t.total:  # this is shown in the progress bar, represents the update times
                ...
                if should_update_times <= 0:
                    ...
                    l_logp, l_alpha, l_int_rew = policy.update_rms(train_collector.buffer[indexes], batch_size=batch_size)
                    l_int_rew = list(l_int_rew.reshape(-1))
                    l_alpha = list(l_alpha.reshape(-1))
                    l_logp = list(l_logp.reshape(-1))
                    ...
                    if test_data is not None:
                        l_logp, l_alpha, l_int_rew = policy.update_rms(test_data,
